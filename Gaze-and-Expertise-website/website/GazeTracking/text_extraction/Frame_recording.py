...
class Screen(object):
    ...
    def capture_Frames(self):
        ...
        while not self.scroll_value:
            ...
            if threshold > 5 and self.new_frame:
                ...
                frame_name = str(self.articleName) + "_" + str(self.n)
                img = Image.frombytes("RGB", window.size,
                                      window.bgra, "raw", "BGRX")
                # path = "../text_extraction/Frames/"
                path = "output/Image_out/Frames/"
                d = datetime.datetime.now() - timedelta(seconds=5)
                output = path + str(frame_name) + "_" + \
                    d.strftime("%H:%M:%S") + ".png"
                img.save(output)
    ...
    def on_stop_recording(self):
        ...
        d = datetime.datetime.now() - timedelta(seconds=5)
        output = path + str(frame_name) + "_" + \
            d.strftime("%H:%M:%S") + "_dummy.png"
        img.save(output)
        if not os.path.exists('output/File_out'):
            os.mkdir('output/File_out/')
        dim_path = "output/File_out/Window_dim.txt"
        dimFile = open(dim_path, "w")
        dimFile.write(str(self.articleName) + " " +
                      str(self.dimensions[2]) + " " + str(self.dimensions[3]))
        dimFile.close()
...
if __name__ == "__main__":
    ...
    if not os.path.exists('output/Image_out/Frames'):
        os.makedirs('output/Image_out/Frames')
    path = "output/Image_out/Frames/"
