...
def convert_date_time(startTime, endTime):
    ...
    start_time = datetime.datetime(1900, 1, 1, int(
        startInfo[0]), int(startInfo[1]), int(startInfo[2]))
    end_time = datetime.datetime(1900, 1, 1, int(
        endInfo[0]), int(endInfo[1]), int(endInfo[2]))
...
def plot_overlay(frame_number, frameName, start_time, end_time):
    ...
    path1 = os.getcwd()
    path1 = path1+"/output/Image_out/Frames/"+frameName
    background = Image.open(path1)

    # opening overlay image
    path1 = os.getcwd()
    path1 = path1+"/output/Image_out/"+frameName+"box.png"
    ...
    path1 = os.getcwd()
    path1 = path1+"/output/Image_out/Overlays/"+frameName
...
def plot_fixation(frame_number, frameName, start_time, end_time, csv_file):
    # Reading gaze file
    df = pd.read_csv("./output/FinalGaze.csv")
    ...
    df['timestamp'] = pd.to_datetime(
        df['timestamp'], format=timefmt, errors='coerce')
    i = 0
    ...
    path = os.getcwd()
    path = path+"/output/Image_out/Fixations/"+frame_number+".txt"
    ...
    while i < df.shape[0]-1:
        if df.iloc[i, 2].time() > start_time.time() and df.iloc[i, 2].time() < end_time.time():
            ...
            if df.iloc[i, 1] <= 100:
                ...
                cv2.circle(image1, center_coordinates,
                ...
                if(d <= 10):
                    if(j == 1):
                        ...
                        with open(path, 'w') as filehandle:
                            filehandle.write(
                                "Time to first fixation in seconds:"+str(time_to_first_fixation.total_seconds())+"\n")
    ...
    csv_file.write(str(count) + "," +
                   str(time_to_first_fixation.total_seconds()) + "," + str(j)+",")
    ...
    path2 = os.getcwd()
    path3 = path2+"/output/Image_out/Fixations/"+frame_number+".png"
    path4 = path2+"/output/Image_out/Fixations/"+frame_number+"_dbscan.png"
    path5 = path2+"/output/Image_out/Fixations/"+frame_number+"_original.png"
    path6 = path2+"/output/Image_out/Fixations/"+frame_number+"_dbscan_flip.png"
    plt.savefig(path4)
    ...
    path1 = os.getcwd()
    path1 = path1+"/output/Image_out/Fixations/"+frame_number+"_original.png"
    background = Image.open(path1)
    path1 = os.getcwd()
    path1 = path1+"/output/Image_out/"+frameName+"box.png"
    overlay = Image.open(path1)
    ...
    new_img = Image.blend(background, overlay, 0.5)
    path1 = os.getcwd()
    path1 = path1+"/output/Image_out/Overlays/fixations_"+frameName
    new_img.save(path1, "PNG")
...
def store_velocity(frame_number, frameName, start_time, end_time, csv_file):
    ...
    path1 = os.getcwd()
    path1 = path1+"/output/Image_out/Velocity/"+frame_number+".txt"
...
def open_images(articleName_csv, count, ratio):
    frameDict = {}
    frameNum = []
    imageNames = os.listdir("./output/Image_out/Frames/")

    # Removing older overlays
    path = "./output/Image_out/Overlays/"
    files = glob.glob(path+'*.png')
    for f in files:
        os.remove(f)

    # Removing older velocity files
    path = "./output/Image_out/Velocity/"
    files = glob.glob(path+'*.txt')
    for f in files:
        os.remove(f)

    # Removing older fixation files
    path = "./output/Image_out/Fixations/"
    files = glob.glob(path+'*.txt')
    for f in files:
        os.remove(f)
    files = glob.glob(path+'*.png')
    ...
    csv_file = open("./output/data_collected.csv", "a+")
    ...
    for frameName in imageNames:
        ...
        csv_file.write(str(articleName_csv)+"," + str(count) +
                       "," + str(ratio)+"," + str(frame_number)+",")
