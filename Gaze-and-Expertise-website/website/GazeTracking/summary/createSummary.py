...
from GazeTracking.summary import new_script
import time
import shutil
from GazeTracking.summary import window_dimensions
import textstat
# import datetime
# import sys
# import mss
...
class Screen(object):
    ...
    def get_Coordinates(self, frameName):
        bboxes = []
        bboxFile = open("./output/File_out/bbox_points.txt", "r")
        for line in bboxFile:
            ...
            if line_parts[0].__contains__(frameName):
                for i in range(1, len(line_parts) - 1):
                    ...
                    coords = [coords[0], coords[1], coords[0] +
    ...
    def clean_Extracted_Text(self, summary_unclean):
        ...
        for i in bad_chars:
            wild_removed = ''.join(
                i for i in non_ascii_removed if i not in bad_chars)
        wild_removed = re.sub(
            '[!{[|]\S+[!\}\]lI1]?|\d+[\]lJ]+', '', wild_removed)
        wild_removed = re.sub(' +', ' ', wild_removed)
        wild_removed = re.sub('@', '', wild_removed)
        wild_removed = re.sub('\n+', '\n', wild_removed)

        # single letters removed
        singles_removed = ' '.join(
            [w for w in wild_removed.split() if (len(w) > 1 and w not in ['a'])])
...
def crMain():
    ...
    sc = Screen()
    frame_list = os.listdir("./output/Image_out/Frames")
    ...
    for filename in sorted(frame_list, key=frame_no):
        if filename.endswith(".png"):
            ...
            articleName1 = (str)(
                articleName)+"_"+str(st_time[0])+"_"+(str)(st_time[1])+"_"+(str)(st_time[2])
            net = "./output/Image_out/Frames/" + filename
            frame = cv2.imread("./output/Image_out/Frames/" + filename)
    ...
    ogtext = open("./output/File_out/ogtext.txt", "w")
    ...
    summaryFinal = open("./output/File_out/summary.txt", "w")
    ...
    config = open("./output/summary_created.txt", "w")
    config.write("1")
    config.close()
    ''' Section break '''
    ...
    info = open("./output/File_out/Info.txt", "w")
