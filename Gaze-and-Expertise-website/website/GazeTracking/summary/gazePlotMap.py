...
import cv2
from GazeTracking.text_extraction import window_dimensions
...
def draw_display(dispsize, imagefile=None):
    ...
    screen = numpy.zeros((dispsize[1], dispsize[0], 3), dtype='float')
...
def bounding_box(imageName, FrameName, display_width, display_height):
    ...
    mask1 = cv2.inRange(roi_hsv, numpy.array(
        [1, 1, 1]), numpy.array([255, 255, 255]))
    mask1 = cv2.morphologyEx(
        mask1, cv2.MORPH_CLOSE, cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (3, 3)))
    ...
    bbox_file = open("./output/File_out/bbox_points.txt", "a")
    ...
    for i, ctr in enumerate(sorted_ctrs):
        if cv2.contourArea(ctr) > 50:
            ...
            if w > 100 and h > 50:
                ...
                cv2.rectangle(image, (x-100, y),
                              (x + w+150, y + h + 50), (0, 255, 0), 2)
                bbox_file.write(
    ...
    cv2.imwrite("./output/Image_out/" + FrameName + 'box.png', image)
    # cv2.waitKey(0)
...
def GazeFrameMap(coordinatesFileName):
    ...
    waitForDummy = True
    while waitForDummy:
        for i_name in imageNames:
            if i_name.__contains__('dummy'):
                waitForDummy = False
        imageNames = os.listdir("output/Image_out/Frames/")
    ...
    for frameName in imageNames:
        ...
        for lines in file:
            ...
            if is_time_between(startTime, endTime, gazeInfo[2]):
                # , gazeInfo[2]))
                gazePoints.append([int(gazeInfo[0]), int(gazeInfo[1]), 1])
        file.close()
        # Pass the gazepoints and the frameName to the heat map generator.
        # print(gazePoints)
        HeatMap_OutFile = "output/Image_out/HeatMaps/" + \
            frameName[0:-4] + "_out.png"
        dimFile = open("output/File_out/Window_dim.txt", "r")
...
def gPMain():
    OutputGazeFile = "output/FinalGaze.csv"
    # Empty the Frames folder
    import glob
    import os

    path = "./output/Image_out/"
    files = glob.glob(path+'*.pngbox.png')
    for f in files:
        os.remove(f)
    # path = "../Integration/Image_out/Frames/"
    path = "./output/Image_out/HeatMaps/"
    files = glob.glob(path + '*.png')
    for f in files:
        os.remove(f)

    # Remove previsous bbox_points file
    if os.path.exists("./output/File_out/bbox_points.txt"):
        os.remove("./output/File_out/bbox_points.txt")
